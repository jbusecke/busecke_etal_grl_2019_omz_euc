...
def remove_seas_simple(ds):
    ...
    return xr.apply_ufunc(
        lambda x, m: (x - m), ds.groupby("time.month"), ds_clim_mean
    )
...
def extract_shape(da, c, dim="st_ocean"):
    ...
    params = xr.Dataset(
        {
            "value": core_val.where(~mask),
            "position": core_pos.where(~mask),
            "extent": thick.isel(new=0),
            "inner": thick.isel(new=1),
...
def extract_EUC_stats(ds, c):
    ...
    if "yu_ocean" in ds.dims:
        ...
        print(
            "Warning: yu_ocean not found....check if it isnt labeled otherwise"
        )
...
def regress_slope(ds, regression_lon):
    ...
    return xr_linregress(
        ds.xu_ocean, ds, dim="xu_ocean", convert_to_dataset=False, nanmask=True
    )
...
def compute_residual(ds, varlist, reference, res_name="residual"):
    ...
    res = ds[reference] - sum(
        [ds[v] for v in residual_list if v in ds.data_vars]
    )
    res.name = res_name
    return res
...
def mom_recombine_budgets(ds):
    ...
    ds = ds.copy()
    budget_vars = [
        a.replace("_tendency", "") for a in ds.data_vars if "tendency" in a
    ]

    # can wrap this in a replace dictionary...but for now hardcode
    for var in budget_vars:
        if (var + "_vdiffuse_impl" in ds.data_vars) and (
            var + "_nonlocal_KPP" in ds.data_vars
        ):
            ds[var + "_vertical_diff"] = (
                ds[var + "_vdiffuse_impl"] + ds[var + "_nonlocal_KPP"]
            )
        else:
            print("No `vertical_diff` generated for %s" % var)

        if ("neutral_diffusion_" + var in ds.data_vars) and (
            "neutral_gm_" + var in ds.data_vars
        ):
            ds[var + "_eddy_combined"] = (
                ds["neutral_diffusion_" + var] + ds["neutral_gm_" + var]
            )
        else:
    ...
    if "o2_advection" in ds.data_vars:
        ...
        ds["o2_advection_recon"] = (
            ds["o2_advection_x_recon"]
            + ds["o2_advection_y_recon"]
            + ds["o2_advection_z_recon"]
        )
        ds["o2_advection_eddy_recon"] = (
            ds["o2_advection"] - ds["o2_advection_recon"]
        )
        ds["o2_advection_recon_w_diff"] = (
            ds["o2_advection_recon"] + ds["o2_vertical_diff"]
        )
        ds["o2_advection_recon_test"] = (
            ds["o2_advection_x"] + ds["o2_advection_y"] + ds["o2_advection_z"]
        )
...
def label_lon_lat_axis(ax, lon=True, axis="x"):
    ...
    if lon:
        ...
        labels[labels < (-180)] = (
            360 + labels[labels < (-180)]
        )  # this should be changed to <= to get 180degE instead of 180degW
    else:
...
def scatter_wrapper(
    ...
    yop,
    ax,
    exclude=["tao", "CMIP5_IPSL-CM5A-LR"],
    invertx=True,
    inverty=True,
):
    ...
    ax.text(
        0.7,
        0.1,
        "r: %.2f (%i%s)" % (reg.rvalue, (1 - reg.pvalue) * 100, "%"),
...
def below_euc_plot(full_ds, shape_ds, highlight_lw=1):
    slim_ds = full_ds.drop(
        [v for v in full_ds.data_vars if ("o2" not in v) and ("jo2" not in v)]
    )
    ...
    mask = xr.ufuncs.logical_and(
        full_ds.st_ocean >= shape_ds.core_pos_z, full_ds.o2 > 80e-6
    )
    ...
    fig, axarr = plt.subplots(
        ncols=4, figsize=[fullwidth * 1.5, 1 * 2], dpi=600
    )

    # Testing how good the reconstruction of the full fluxes works
    ax = axarr.flat[0]
    ds_mean.o2_advection.mean("time").plot(
        ax=ax, label="advection", linewidth=highlight_lw, color="0.5"
    )
    ds_mean.o2_advection_recon_test.mean("time").plot(
        ax=ax, label="advection from full fluxes"
    )
    (ds_mean.o2_advection - ds_mean.o2_advection_recon_test).mean("time").plot(
    ...
    (ds_mean.o2_advection + ds_mean.o2_eddy_combined).mean("time").plot(
        ax=ax, label="advection+eddy"
    )
    ds_mean.o2_eddy_combined.mean("time").plot(ax=ax, label="eddy")
    ds_mean.o2_advection_recon.mean("time").plot(
        ax=ax,
        label="advection (large scale)",
        linewidth=highlight_lw,
        color="0.5",
    )
    (ds_mean.o2_advection - ds_mean.o2_advection_recon).mean("time").plot(
        ax=ax, label="advection - advection(large scale)"
    )
    ...
    ax = axarr.flat[2]
    ds_mean.o2_advection_recon.mean("time").plot(
        ax=ax, label="advection (large scale)"
    )
    ds_mean.o2_advection_x_recon.mean("time").plot(
        ax=ax,
        label="x advection (large scale)",
        linewidth=highlight_lw,
        color="0.5",
    )
    ds_mean.o2_advection_y_recon.mean("time").plot(
        ax=ax, label="y advection (large scale)"
    )
    ds_mean.o2_advection_z_recon.mean("time").plot(
        ax=ax, label="z advection (large scale)"
    )
    ...
    ds_mean.o2_advection_x_recon_vel.mean("time").plot(
        ax=ax,
        label="x advection (divergence)",
        linewidth=highlight_lw,
        color="0.5",
    )
    ds_mean.o2_advection_x_recon_tracer.mean("time").plot(
        ax=ax, label="x advection (tracer)"
    )
...
def budget_plot(
    ...
    for source, ax, dx in zip(
        ["CM21deg", "CM26"], axes, np.array([1, 1 / 10]) * 111e3
    ):
        ...
        if var in ds.data_vars:
            ...
            if metric == "mean":
                ...
                ds_plot.mean("time").plot.contourf(
                    levels=31, ax=ax, **plot_kwargs
                )
            elif metric == "std":
                plot_kwargs["vmax"] = 6e-9
                plot_kwargs["vmin"] = -plot_kwargs["vmax"]
                ds_plot.std("time").plot.contourf(
                    levels=30, ax=ax, **plot_kwargs
                )
        ...
        [
            axis_arrow(ax, is_lon, is_name, arrowprops=arrow_dict)
            for is_name, is_lon in islands.items()
        ]
...
def eof_plot(
    ...
    var_factors={"o2": 1e6},
    var_plot_kwargs={
        "o2": {"robust": True, "yincrease": False},
    ...
    for xi, var in enumerate(names_eofs):
        ...
        for ai, (mode, ax) in enumerate(zip(ds_eof.mode, axes)):
            h = plot_ds.sel(mode=mode).plot.contourf(
                ax=ax, levels=15, **kwargs
            )
            cb = h.colorbar
            ...
            ds["u"].mean("time").plot.contour(
                ax=ax, levels=u_levels, colors=u_color, **cont_kwargs
            )
            ds["o2"].mean("time").plot.contour(
                ax=ax, levels=o2_levels, colors=o2_color, **cont_kwargs
            )
            ds["pot_rho_0"].mean("time").plot.contour(
                ax=ax, levels=rho_levels, colors=rho_color, **cont_kwargs
            )

            regress_field_absolute(
                ds["o2"], ds_eof["pc"].sel(mode=mode)
            ).plot.contour(
                ax=ax,
                levels=o2_levels,
                colors=o2_color,
            ...
            regress_field_absolute(
                ds["pot_rho_0"], ds_eof["pc"].sel(mode=mode)
            ).plot.contour(
                ax=ax,
                levels=rho_levels,
                colors=rho_color,
            ...
            ax.text(
                ...
                0.05,
                "%s percent"
                % int(abs(ds_eof["variance_fraction"].sel(mode=mode)) * 100),
                transform=ax.transAxes,
            )
            ax.set_xticks(np.arange(-220, -60, 40))
            label_lon_lat_axis(ax, lon=True, axis="x")
...
def calculate_momentum_budget(ds):
    ...
    combo["u_du_dx"] = grid.interp(
        -combo["du_dx"] * grid.interp(ds.u, "X"), "Y"
    )
    combo["v_du_dy"] = grid.interp(
        -combo["du_dy"] * grid.interp(ds.v, "Y"), "X"
    )

    combo["hor"] = combo["u_du_dx"] + combo["v_du_dy"]
    combo["hor"].attrs[
        "long_name"
    ] = "Zonal Velocity tendency due to hor divergence of momentum"
...
def rewrap_eof(ds, ds_old):
    ...
    coords = [("mode", np.arange(n_mode))] + [
        (k, v.data) for k, v in coords.items() if k in ds_old.dims
    ]
...
def rewrap_pc(ds, time):
    ...
    coords = [("mode", np.arange(n_mode))] + [
        (k, v.data) for k, v in coords.items() if k in time.dims
    ]
    return xr.DataArray(ds, coords=coords)
...
def eof_wrapper(ds, fields, weight_fields, n=4, normalize=True):
    ...
    if len(data) != len(weights):
        raise ValueError(
            "fields and weight fields need to have the same number of inputs"
        )
    ...
    if multivariate and not normalize:
        raise RuntimeError(
            "For multivariate eofs it is suggested to normalize all variables with their global variance. \
    ...
    if multivariate:
        eof = xr.Dataset(
            {k: rewrap_eof(v, ds[k]) for k, v in zip(fields, eof)}
        )
    else:
        eof = xr.Dataset({fields[0]: rewrap_eof(eof, ds[fields[0]])})

    pc = rewrap_pc(pc, ds.time).to_dataset(name="pc")
    variance_fraction = rewrap_eig(variance_fraction).to_dataset(
...
def interp_all(grid, ds, target="center"):
    ...
    ds_new = xr.Dataset()
    warnings.warn(
        "Frozen Version: Up to date version is maintained in xarrayutils",
        DeprecationWarning,
    )
